  ...
  window.Artboard = function() {
    ...
    const history = {
      ...
      play() {
        ...
        if (history.isPlaying || !history.timeline.length) {
          return false;
        } else {
      ...
      saveAs(str, devCallback) {
        if (!history.isPlaying && history.timeline.length && /^[a-zA-Z0-9_$]+$/.test(str)) {
      ...
      restore(data, devCallback) {
        if (history.isPlaying) {
          return false;
        } else if (data && !(data instanceof Event)) {
      ...
      undo() {
        if (!history.isPlaying && history.undoCount !== history.timeline.length) {
          history.undoCount++;
          history.change();
          return true;
        } else {
      ...
      redo() {
        if (!history.isPlaying && history.undoCount > 0) {
          history.undoCount--;
          history.change();
          return true;
        } else {
    ...
    function mouseMove(e) {
      e.preventDefault();
      e.stopPropagation();
      if (history.isPlaying || !isMouseDown) { return; }
      x = [e.pageX - canvasLeftToPageLeft];
      y = [e.pageY - canvasTopToPageTop];
      draw();     
    }
