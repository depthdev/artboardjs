...
(function(window, document) {
  ...
  window.Artboard = function() {
    ...
    const history = {
      ...
      undoCount: 0,
      save() {
        localStorage.artboard = JSON.stringify({width:canvas.width,height:canvas.height,timeline:history.timeline});
      },
      saveAs(str, devCallback) {
        if (/^[a-zA-Z0-9_$]+$/.test(str)) {
          localStorage[`artboard_${str}`] = JSON.stringify({width:canvas.width,height:canvas.height,timeline:history.timeline});
      ...
      restore(data, devCallback) {
        if (data && !(data instanceof Event)) {
          ...
          if (typeof data === 'string') {
            if (localStorage[`artboard_${data}`]) {
              const parsedData = JSON.parse(localStorage[`artboard_${data}`]);
        ...
        else if (localStorage.artboard) {
          const parsedData = JSON.parse(localStorage.artboard);
          canvas.width = parsedData.width;
          canvas.height = parsedData.height;
          history.timeline = parsedData.timeline;
          history.undoCount = 0;
      ...
      undo() {
        ...
        return history.currentStateForDev();
      },
      redo() {
    ...
    function resetHard(devObj) {
      ...
      canvas.height = o.height || window.innerHeight;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
